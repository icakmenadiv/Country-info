...
const state = {countries:[],updates:[],selected:null,snapshotAt:null,structure:{countries:{}}};
...
function structureFor(code){return state.structure?.countries?.[code]||null}
function classifyItem(item){
  const s=structureFor(item.country_code);
  if(!s||!s.verified)return {status:'unknown',merged_into:null};
  const active=new Set(s.active_titles||[]), retired=new Set(s.retired_titles||[]);
  ...
  const parent=(s.active_titles||[]).filter(t=>item.title.startsWith(t+' > ')).sort((a,b)=>b.length-a.length)[0]||null;
  if(parent)return {status:'merged',merged_into:parent};
...
function renderNext(){
  ...
  $('#nextTarget').innerHTML=`
    ...
    <p class="hint">현재 OCIS에서 독립 운영되는 active 항목만 추천합니다. 통합된 과거 세부항목은 제외합니다.</p></div>`;
}
function renderCountries(){
  ...
  $('#countryGrid').innerHTML=list.map(c=>{
    const active=activeCountryItems(c.code).length, merged=mergedCountryItems(c.code).length, unknown=unknownCountryItems(c.code).length;
    return `<button class="country-card ${state.selected===c.code?'active':''}" data-code="${c.code}">
      ...
      <div class="card-stat"><span>구조확인필요</span><b>${unknown}</b></div>
    </button>`;
...
function renderDetail(){
  ...
  $('#detailSub').textContent=`현재 운영 항목은 수정일 오래된 순 · 통합/폐지 항목은 자동 추천 제외`;
...
async function init(){
  try{
    const [s,catalog,structure]=await Promise.all([loadJSON('data/state.json'),loadJSON('data/catalog.json'),loadJSON('data/structure.json',{countries:{}})]);
    state.countries=s.countries||[];
    state.updates=expandCatalog(catalog);
    state.structure=structure||{countries:{}};
